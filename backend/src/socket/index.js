...
import { messages, conversations, students, tutors, sessions, callEvents, communityMessages, users } from '../db/schema.js';
import { eq, and, sql } from 'drizzle-orm';
import { moderateCommunityMessage } from '../services/ai.service.js';
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('send_community_message', async ({ content }) => {
            ...
            try {
                ...
                // Broadcast with sender info so frontend CommunityBubble can display name/initials
                io.to('community_room').emit('new_community_message', {
                    ...newMsg,
                    sender: senderUser ? { id: senderUser.id, name: senderUser.name } : null,
                });
            } catch (err) {
