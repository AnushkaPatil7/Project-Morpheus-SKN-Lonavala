...
export async function submitTest(req, res) {
    ...
    try {
        ...
        // Return the real evaluation so the frontend can show dynamic results
        return res.status(200).json({
            message: evaluation.passed
                ? 'You passed the competence test! Your profile is now pending admin approval.'
                : 'Test attempt completed. You can retake the test.',
            evaluation,
        });
    } catch (error) {
