...
import { useLearningStore, deriveStability, type TopicState } from "../../store/learning.store";
...
const stabilityConfig = {
    stable: { label: "Stable", icon: CheckCircle2, text: "text-emerald-400", bg: "bg-emerald-500/10", border: "border-emerald-500/30", glow: "hover:shadow-emerald-500/20", ring: "#22c55e", bar: "bg-emerald-500", badge: "bg-emerald-500/20 text-emerald-400" },
    shaky: { label: "Shaky", icon: AlertCircle, text: "text-amber-400", bg: "bg-amber-500/10", border: "border-amber-500/30", glow: "hover:shadow-amber-500/20", ring: "#f59e0b", bar: "bg-amber-500", badge: "bg-amber-500/20 text-amber-400" },
    fragile: { label: "Fragile", icon: XCircle, text: "text-red-400", bg: "bg-red-500/10", border: "border-red-500/30", glow: "hover:shadow-red-500/20", ring: "#ef4444", bar: "bg-red-500", badge: "bg-red-500/20 text-red-400" },
};
...
function TopicCard({ topic, onRevise, onFlashcards }: {
    ...
    const lastPracticedLabel = topic.lastPracticed
        ? (() => {
            const diff = Date.now() - new Date(topic.lastPracticed).getTime();
    ...
    return (
        <div
            className={cn("rounded-2xl border p-4 transition-all duration-200 hover:shadow-lg cursor-pointer select-none", cfg.bg, cfg.border, cfg.glow)}
        ...
        >
            <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                        ...
                        {topic.flashcardSessions > 0 && (
                            <span className="text-[10px] text-morpheus-accent">·  {topic.flashcardSessions} session{topic.flashcardSessions > 1 ? "s" : ""}</span>
                        )}
                    </div>
                    <p className="font-display font-semibold text-morpheus-text text-sm leading-tight">{topic.name}</p>
                    <p className="text-[10px] text-morpheus-muted mt-1">Tap to see revision options</p>
                </div>
            </div>
            ...
            <div className="mt-3 grid grid-cols-3 gap-2">
                <div className="text-center">
                    ...
                    <p className="text-xs font-semibold text-morpheus-text">{lastPracticedLabel}</p>
                </div>
            ...
            {showActions && (
                <div className="mt-4 pt-3 border-t border-white/10 flex gap-2 flex-wrap" onClick={(e) => e.stopPropagation()}>
                    ...
                    <button
                        onClick={() => window.location.href = `/student/discovery?subject=${encodeURIComponent(topic.subject)}`}
...
export default function ConceptStabilityPage() {
    ...
    const resetLearningData = useLearningStore((s) => s.resetLearningData);

    const [filter, setFilter] = useState<"all" | Stability>("all");
    const [subjectFilter, setSubjectFilter] = useState<string>("All");
    const [lastRefreshed] = useState(new Date());

    const subjects = useMemo(() => ["All", ...Array.from(new Set(topics.map((t) => t.subject)))], [topics]);

    const stableCount = topics.filter((t) => t.stability === "stable").length;
    const shakyCount = topics.filter((t) => t.stability === "shaky").length;
    ...
    const overallScore = Math.round(topics.reduce((a, t) => a + t.score, 0) / topics.length);
        ...
        <StudentLayout>
                ...
                <div className="flex items-center gap-3 mb-1">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-morpheus-muted">
                            Live health check · Updated {lastRefreshed.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-8">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-morpheus-muted">
                            {fragileCount > 0
                                ...
                                : shakyCount > 0
                                    ? `${shakyCount} topic${shakyCount > 1 ? "s" : ""} need revision`
                                    : "All topics stable! Keep it up 🎉"}
                        </p>
                        ...
                        <p className="text-[10px] text-morpheus-muted mt-1.5">
                            {topics.filter(t => t.flashcardSessions > 0).length} topics practiced · {topics.reduce((a, t) => a + t.flashcardSessions, 0)} total sessions
                        </p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3 sm:w-auto">
                    {[
                        { label: "Stable", count: stableCount, color: "text-emerald-400", bg: "bg-emerald-500/5 border-emerald-500/30", filterVal: "stable" as Stability },
                        { label: "Shaky", count: shakyCount, color: "text-amber-400", bg: "bg-amber-500/5 border-amber-500/30", filterVal: "shaky" as Stability },
                        { label: "Fragile", count: fragileCount, color: "text-red-400", bg: "bg-red-500/5 border-red-500/30", filterVal: "fragile" as Stability },
                    ].map((s) => (
                        <div key={s.label} onClick={() => setFilter(filter === s.filterVal ? "all" : s.filterVal)}
            ...
            <div className="flex flex-wrap gap-2 mb-4 items-center">
                ...
                <div className="flex gap-1 flex-wrap">
                    {(["all", "stable", "shaky", "fragile"] as const).map((f) => (
                        <button key={f} onClick={() => setFilter(f)}
                            ...
                            {f === "all" ? `All (${topics.length})` : `${f} (${topics.filter((t) => t.stability === f).length})`}
                        </button>
                    ))}
                </div>
                ...
                <button onClick={() => resetLearningData()} className="flex items-center gap-1.5 text-xs text-morpheus-muted hover:text-red-400 font-medium transition-colors">
                    <RotateCcw size={12} /> Reset data
                </button>
            </div>

            <p className="text-xs text-morpheus-muted mb-6">
                Score updates in real-time after each flashcard session · <span className="font-medium">Tap any card</span> to start revision
            </p>
            ...
            {filtered.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {filtered.map((t) => (
                        <TopicCard key={t.slug} topic={t} onRevise={handleRevise} onFlashcards={handleFlashcards} />
                    ))}
                </div>
            )}
            ...
            {(filter === "all" || filter === "fragile") && fragileCount > 0 && (
                <div className="mt-8 rounded-2xl border border-red-500/30 bg-red-500/5 p-4 flex flex-col sm:flex-row items-start sm:items-center gap-3 justify-between">
                    <div>
                        <p className="font-display font-semibold text-morpheus-text text-sm">🚨 {fragileCount} fragile topics need immediate attention!</p>
                        <p className="text-xs text-morpheus-muted mt-0.5">Revising these now can significantly improve your exam performance.</p>
                    </div>
                    <div className="flex gap-2 flex-wrap shrink-0">
                        <button onClick={() => setFilter("fragile")}
                        ...
                        <button
                            onClick={() => handleRevise(topics.filter((t) => t.stability === "fragile").sort((a, b) => a.score - b.score)[0]?.slug ?? "")}
            ...
            {fragileCount === 0 && stableCount === topics.length && (
                <div className="mt-8 rounded-2xl border border-emerald-500/30 bg-emerald-500/5 p-4 text-center">
                    <p className="text-emerald-400 font-semibold">🎉 All topics are stable! Great work keeping up with your revisions.</p>
                </div>
            )}
        </StudentLayout>
    );
